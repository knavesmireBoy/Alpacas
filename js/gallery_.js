	...
	var enter,
		...
		listen = function (createDiv, createButton) {
			var lis = _.toArray($('thumbnails').getElementsByTagName('li')),
				...
				makeNavigator = function(iterator, element) {
                    var ret = makeNavListener(getRoute(iterator), element);
					_.each(['getCurrent', 'getNext', 'getCollection'], _.partial(gAlp.Util.proxy, iterator), ret);
					return ret;
                    /*
                    makeNavigator = function(iterator, element){
                        ret.setSubject(iterator);
                    }
                    */
				};
			////////////////////////////////////////////////////////////////////////////////////
						...
						machSchau = function (e, bool) { //memo
							...
							if (!bool) {
                                var predicate = gAlp.Util.getPredicate(navigator.getCurrent(), isLscp),
                                    iterator = iteratorBridge(getDomTargetImg(navigator.getCurrent()), predicate, lis);
								...
								doListeners = _.partial(best, _.partial(thunk, once(1)));
								doAni = _.partial(best, _.partial(thunk, once(1)));
                                iterator = iteratorBridge(getDomTargetImg(navigator.getCurrent()), filterSlides, lis);
								navigator = makeNavigator(iterator, thumbnails);
								doNull();
                                doNull = null;
