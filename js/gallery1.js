		...
		listen = function (createDiv, createButton) {
			var isImg = gAlp.Util.validator('Please click on an image', _.partial(isNodeName, 'IMG')),
				...
				makeNavListener = function(command, element) {
                    ...
                    locationhandler = locationhandler || _.compose(addMyEvent(clickHandler, delegateLocation), _.identity)(element);
				...
				makeNavigator = function(tgt, predicate, element) {
                        var iterator = iteratorBridge(tgt, predicate, lis),
                            ret = makeNavListener(getRoute(iterator), element);
				...
				var navigator = makeNavigator(tgt, always(true), this),
					...
					machSchau = function (e, bool) {
						...
						machSchau = function (e, bool) { //memo
							...
							if (!bool) {
                                var predicate = gAlp.Util.getPredicate(navigator.getCurrent(), isLscp);
								//navigator = getPlayIterator(navigator.getSubject(), isLscp, thumbnails);
                                navigator = makeNavigator(getDomTargetImg(navigator.getCurrent()),predicate, thumbnails);
								...
								var config = {
									...
									//play_iterator = navigator.getSubject(),
								...
								doListeners = _.partial(best, _.partial(thunk, once(1)));
								doAni = _.partial(best, _.partial(thunk, once(1)));
								navigator = makeNavigator(getDomTargetImg(navigator.getNext(true)), filterSlides, thumbnails);
								doNull();
                                doNull = null;
